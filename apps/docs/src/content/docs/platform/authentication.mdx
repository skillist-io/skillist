...
| Method | Where |
|--------|-------|
| GitHub OAuth | [skillist.io/login](https://skillist.io/login) |
| Google OAuth | [skillist.io/login](https://skillist.io/login) |
| Passkey | Login page or [Account](https://skillist.io/account) |
...
Signed-in users manage personal auth at [skillist.io/account](https://skillist.io/account):
...
Session cookies are used by the web app. Production sessions use cross-subdomain cookies on `.skillist.io` so `skillist.io` and `api.skillist.io` share the same session. The CLI and API keys use Bearer tokens after `skillist login`.

Protected API routes live under `/v1/*` and require a valid session or org API key.

## MCP OAuth

MCP clients use a separate OAuth flow — see [OAuth authentication](/mcp/oauth/). Sign-in still happens at `skillist.io/login`.
