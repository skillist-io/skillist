...
<DocAlert title="Requires sign-in" client:load>
  Inventory is available at [console.skillist.io/inventory](https://console.skillist.io/inventory) for authenticated org members. API routes live under `/v1/orgs/{orgId}/inventory`.
</DocAlert>
...
```
https://skillist.io/{registryOrgSlug}/{registryRepo}
```
...
- [Delivery URLs](/platform/delivery/) — canonical `skillist.io/{org}/{repo}` paths
