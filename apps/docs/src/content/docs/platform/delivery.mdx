---
title: Delivery URLs
description: GitHub-style public URLs at skillist.dev/{org}/{repo} for skill pages and raw agentskills.io delivery.
---
...
<DocAlert title="Browse vs delivery" client:load>
  The public **registry index** stays at [skillist.dev/registry](https://skillist.dev/registry). Individual skills live at `skillist.dev/{org}/{repo}` — not under `/registry/`.
</DocAlert>
...
Cloudflare zone routes cannot place wildcards mid-path (e.g. `skillist.dev/*/SKILL.md`), so delivery is implemented in two layers:
