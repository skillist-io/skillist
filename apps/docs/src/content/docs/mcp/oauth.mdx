...
description: Skillist uses the Better Auth MCP plugin so OAuth-capable MCP clients can sign in with your Skillist account.
---

Skillist acts as an OAuth provider for MCP clients using the [Better Auth MCP plugin](https://better-auth.com/docs/plugins/mcp). This lets Cursor and other OAuth-aware clients obtain Bearer tokens tied to your Skillist user.
...
Better Auth also serves metadata under `/api/auth/.well-known/…` for clients that follow the auth server path.
...
3. You sign in at **[skillist.dev/login](https://skillist.dev/login)** (GitHub, Google, magic link, or email OTP).
...
- OAuth tables: `oauth_application`, `oauth_access_token`, `oauth_consent` (Better Auth OIDC schema)
