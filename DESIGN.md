...
The identity pairs a **wordmark and one mark**. The wordmark is SKILLIST set in Inter SemiBold capitals at `+0.14em` tracking — the Equipment-Label voice, one step larger — speaking in the exact register the product labels its switches and readouts with, the most literal form of "practice what you preach" the brand has. The **horizontal logo** sets the socket (below) to the left of the wordmark as a lockup, used in the site header and footer; the wordmark stands **alone** where the mark already appears nearby (the console's icon rail sits directly under it) or where space is too tight for both. Both carry **no colour**; the single ink inverts with the ground (ink on light, paper on dark). Assets generate from the committed outlines in `scripts/brand/wordmark.json` and the socket geometry in `scripts/brand/geometry.mjs` via `pnpm brand:assets`. Do not hand-set a new one. The lockup is composed in code by `SkillistLogo withMark` (`packages/ui`), which draws `SkillistMark` from the same geometry.

(Two pictorial marks preceded this and are retired in git history: the letterform `ill` cut from the middle of Sk**ill**ist — retired because a standalone mark that spells "ill" argues with a brand built on trust — and a three-rule "list" glyph, retired as decoration the wordmark didn't need. A Bold **S** stamp then served the square contexts until it was replaced by the socket; the S outline stays in `wordmark.json` for history.)

- **The socket.** The mark is the **socket**: four congruent L corner-tiles framing an empty centre. The tiles are the registry; the socket is the slot the next skill drops into. It is built on two mirror axes, never rotation — a chiral pinwheel of four same-handed arms is the swastika topology, so the tiling is reflected, not spun. Single ink, squared, no gradient. In the horizontal lockup it sits to the left of the wordmark in the page ink (transparent seams and centre, so the ground shows through); where a word cannot go — favicon, app icon, avatar — it stands alone knocked out of a filled square tile. Geometry is parametric in `scripts/brand/geometry.mjs` (`socketTiles`); never redraw it by hand.
- **Inversion.** The tile inverts the page: ink tile with a paper aperture on light, paper tile with an ink aperture on dark. Raster icons that cannot adapt take the ink tile.
...
- **Small sizes.** Below roughly 80px of width the wordmark's tracking channels close up; use the socket stamp instead. The socket has its own floor: below ~28px the seams smear to grey mush, so favicons and small `.ico` layers ship a deliberately simplified variant — the socket with its seams removed, a plain square aperture (same outer bounds, same socket). The generator switches to it automatically (`SOCKET_MIN_PX`, `socketAperture`); the full seamed mark is for 32px and up. The wordmark never renders below 12px cap height.
- **Never** recolour the wordmark, track it tighter or looser, set it in another weight or case, add any symbol other than the socket beside it (the socket lockup is the one sanctioned pairing), or put the violet in the logo — in the brand, colour is spent nowhere so it means everything in the product. Distributed assets ship the letterforms as **outlines**, because a logo that depends on the viewer having Inter installed is not a logo.
...
- **Do** use the signal violet only for live/realtime and active/selected state, and cap it at ≤10% of any screen. The brand carries no violet at all — the wordmark and the socket stamp are single-ink — so every violet pixel on screen is state, never identity.
