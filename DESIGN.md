...
The identity is **wordmark-only**: SKILLIST set in Inter SemiBold capitals at `+0.14em` tracking — the Equipment-Label voice, one step larger. The logo speaks in the exact register the product labels its switches and readouts with, which is the most literal form of "practice what you preach" the brand has. The horizontal logo stands alone with **no colour and no symbol beside it**; its single ink inverts with the ground (ink on light, paper on dark). Where the word cannot go, a separate squared stamp — the socket, below — carries the identity. Assets generate from the committed outlines in `scripts/brand/wordmark.json` and the socket geometry in `scripts/brand/geometry.mjs` via `pnpm brand:assets`. Do not hand-set a new one.
...
- **The socket stamp.** Where a word cannot go — favicon, app icon, avatar — the stamp is the **socket**: four congruent L corner-tiles framing an empty centre, knocked out of a filled square tile. The tiles are the registry; the socket is the slot the next skill drops into. It is built on two mirror axes, never rotation — a chiral pinwheel of four same-handed arms is the swastika topology, so the tiling is reflected, not spun. Single ink, squared, no gradient. Geometry is parametric in `scripts/brand/geometry.mjs` (`socketTiles`); never redraw it by hand.
